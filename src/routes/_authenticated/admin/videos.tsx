...
import { WELCOME_VIDEO_SLOT, formatBytes, toEmbedUrl } from "@/lib/videos";
...
function AdminVideos() {
  ...
  async function makeLive(id: string) {
    // Retire every other entry, then activate the chosen one.
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          >
            ...
            <Field label="Video link (YouTube, Vimeo, Google Drive, or direct MP4)">
              <input name="external_url" placeholder="https://youtube.com/watch?v=…" className={inputClass} required />
            </Field>
            <div>
              <button className="btn btn-outline-ink">Publish link</button>
              <p className="mt-2 text-xs text-muted-foreground">
                YouTube and Vimeo links play right on the page. Any link can be swapped or retired
                below at any time.
              </p>
            </div>
          </form>
      ...
      <Panel title="Video history" description="Only one video is live at a time.">
        {videos.data && videos.data.length > 0 ? (
          ...
          <EmptyNote>
            No videos yet. Families currently see the placeholder message.
          </EmptyNote>
        )}
      </Panel>
    </div>
